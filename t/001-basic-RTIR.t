...
diag 'Reverse history order on RTIR display pages';
{
    ...
    my $rendered_txn_ids = sub {
        ...
        $agent->get_ok(
            "$path?ForceShowHistory=1;ReverseTxns=$reverse;id=$id",
            "loaded $path for ticket $id with ReverseTxns=$reverse"
        );
        return [
            map  { $_->attr('data-transaction-id') }
            grep { !$_->matches('.end-of-history-list') } $agent->dom->find('div.transaction')->each
        ];
    };
    ...
    for my $path ( sort keys %pages ) {
        ...
        cmp_ok( scalar @$asc, '>=', 2, "$path renders at least 2 transactions for ticket $id" );
        is_deeply( $desc, [ reverse @$asc ], "$path renders history in the reverse order for ticket $id" );
    }
    ...
    for my $mode (qw/delay click/) {
        ...
        for my $path ( sort keys %pages ) {
            my $id = $pages{$path};
            $agent->get_ok( "$path?ReverseTxns=DESC;id=$id", "loaded $path for ticket $id in '$mode' mode" );
            like(
                $unescaped_content->(),
